...
\]
ovvero il segnale in uscita è proporzionale alla differenza tra i segnali in ingresso. Pertanto si costruirà un circuito in cui i valori di $R_1$ e $R_1'$ e di $R_2$ e $R_2'$ siano il più simili possibile, e si sceglierà $R_2 / R_1 =: G = 5$. Per ottenere questo scopo, si utilizzeranno dei resistori per $R_1$ e $R_2$, e dei trimmer per $R_1'$ e $R_2'$, la cui resistenza è regolabile con più finezza mediante un cacciavite.
...
Si misurano quindi l'amplificazione di modo comune $A_c$ e l'amplificazione differenziale $A_d$, nonché il Common Mode Rejection Ratio (CMRR), $\rho = \abs{A_d / A_c}$, e li si confronta con i loro valori attesi dati i componenti del circuito e con quelli caratteristici di un amplificatore differenziale ideale.

Per misurare $A_c$ si invia lo stesso segnale ai due ingressi, ossia un segnale sinusoidale di frequenza $f$ e ampiezza $V_i$, che è abbastanza grande per osservare l'attenuazione data dal piccolo valore di $A_c$ e minore di \SI{10}{V} (si veda l'istantanea dell'oscilloscopio in figura~\ref{fig:Ac_differenziale}), valore oltre il quale si osservano distorsioni. Si riportano in tabella~\ref{tab:Ac_differenziale} le misure compiute a diverse frequenze. Il valor medio con associata la deviazione standard è
\[
...
Per misurare $A_d$ si devono invece ricavare singolarmente le amplificazioni $A_1$ e $A_2$, inviando un solo segnale ad un ingresso e mettendo a terra l'altro. Inviando il segnale a $v_1$ e mettendo a terra $v_2$ si avrà un amplificatore invertente, dunque ci si aspetta di ottenere 
\[
A_1 = -\frac{R_2}{R_1}.
\]
Inviando il segnale a $v_2$ e mettendo a terra $v_1$, invece, si avrà un amplificatore non invertente, dunque ci si aspetta di ottenere 
\[
...
\end{aligned}
\]
Il valore medio dell'amplificazione differenziale, con associata incertezza data dalla deviazione standard, è
\[
A_d = -4.53 \pm 0.05,
\]
compatibile con il valore atteso dati i componenti di $A_d^\text{\tiny{(atteso)}} = -4.69 \pm 0.05$. Si può confrontare anche il valore misurato di $A_c$ con quello atteso $A_c^\text{\tiny{(atteso)}} = A_1 + A_2 = -0.004 \pm 0.010$, osservando che essi sono compatibili; si sottolinea che l'errore sul valore previsto di $A_c$ è relativamente ingente proprio perché ottenuto da una differenza di grandezze i cui valori sono quasi uguali.

Si nota inoltre che il valore di $A_c$ è prossimo allo zero, mentre quello di $A_d$ al valore pari al guadagno $G = 5$, secondo il quale sono state scelte le resistenze all'inizio.

Si stima quindi il CMRR 
\[
\rho = \abs{\frac{A_d}{A_c}} = \SI{1.51 \pm 0.06 e3}{},
\]
compatibile con il valore atteso di $\rho \simeq \SI{1.2 e3}{}$, che è fornito qui senza incertezza in quanto essa non è stimabile, dal momento che l'errore relativo sul valore atteso di $A_c$ è maggiore del $200\%$, come visto prima. Si osserva che questo valore è dello stesso ordine di grandezza di quello riportato nel datasheet, ossia $\rho = 65 \div 80 \SI{}{dB}$; nell'amplificatore differenziale ideale, invece, si dovrebbe avere $\rho \rightarrow \infty$.
...
Si ha cura di rispettare la disuguaglianza $V_o^\text{max} < \SI{15}{V}$ per mantenere l'amplificatore nella regione di lavoro, in quanto esso è alimentato, come nella sezione precedente, con una tensione di $\pm \SI{15}{V}$.
...
Si può stimare la frequenza dell'inviluppo con i cursori (e avendo cura di impostare la modalità singola del \texttt{TRIGGER}), ottenendo $f_\text{inv} = \SI{28.3 \pm 0.8}{Hz}$; si confronta questo valore con quello previsto di 
\[
...
Si riporta invece in figura~\ref{fig:battimenti_uscita} lo screenshot del segnale in uscita: è evidente l'inviluppo dei due, che procede alla frequenza $f_\text{inv}$ stimata precedentemente.
